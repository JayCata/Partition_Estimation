...
function generate_trend!(panel_data, params, num_I, T, Tstar, Γ)
    ...
    Yold = Yold = 1 .+ [(s/num_I) for s in range(1,num_I)] + rand(eps_dis, num_I)
    Y = Yold
...
for s in range(1, num_sims)
    ...
    drop_idx = argmax([x[1][1] for x in treatment_temp])
    ztildes_errors = []
    ztildes = [] 
    i_order = []
    t_order = []
...
altprobs = [[0,(.5 *.59375)/ ((.5 *.59375 + .2 *.203125)), (.2 *.203125)/ ((.5 *.59375 + .2 *.203125))][group_order] for i in range(1,num_sims)]
treat_alt_est = [sum([x * y for (x,y) in zip(prob, eachrow(treat_estimate))]) for (prob, treat_estimate) in zip(altprobs, treat_estimates)]
treat_alt_ses = [replace(sqrt.(sum([x * y .^ 2 * x for (x,y) in zip(prob, eachrow(treat_se))])), NaN => Inf) for (prob, treat_se) in zip(altprobs, treat_ses)]
...
true_treat = [( (1 + 2*(t==T)) * params[2][j] * (t >= Tstar) + (3-t+1) * (j==1) ) for j in group_order, t in range(1, T)]


α = .05 
z95 = quantile(Normal(0,1), (1 - (α/2)))
true_agg_treat = [0, 0, 0, 0, .4638888888888888, 1.391666666666]
for s in range(1, num_sims)
