...
tot_trend_reg = reg(panel_df, trend_form, contrasts = Dict(:t => DummyCoding(base = Tstar-1)), Vcov.cluster(:i))
...
ind_treat = combine(groupby(panel_df[:, ["i", "treat"]],  ["i"]), :treat => mean)
 for (j, idx) in enumerate(individual_partition)
     ...
     push!(prob_weights, prob_temp)
     temp_var = diag(trend_regs[j].vcov[T+1:end, T+1:end])
     push!(vars, insert!(temp_var, Tstar-1, Inf))
 end
 #prob_weights = [(.8 *.203125)/.5, (.5 *.59375)/.5, (.2 *.203125)/.5][group_convert]
 prob_weights = temp_prob_num ./ temp_prob_den
 β_j = vcat(β_j'...)
 β = (prob_weights' * β_j)[1, :]
 Δvars = []
...
#      push!(Δvars, prob_weights' * diagm(var_jt) * prob_weights)
#  end
...
plotpath = string(folder, "\\all_grp_1.png")
...
#     end
# end
empty_groups = prob_weights .== 0
remove_groups = [x[1] for  x in findall((empty_groups) .== 1)]
# Exclude Group
...
for (j, idx) in zip(p_idx, p_individual_partition)
    ...
    prob_temp = mean(ind_treat.treat_mean[( ind_treat[!,"i"] .∈ Ref(idx)) ]) * ((length(idx)/num_I)/ mean(ind_treat.treat_mean))
    push!(pprob_weights, prob_temp)
    temp_var = diag(trend_regs[j].vcov[T+1:end, T+1:end])
    push!(pvars, insert!(temp_var, Tstar-1, Inf))
end
 pβ_j = vcat(pβ_j'...)
 trueprob_weights = 
 pβ = (pprob_weights' * pβ_j)[1, :]
 pΔvars = []
...
for t in range(1, T-1)
    ...
    push!(pΔvars, pprob_weights' * diagm(var_jt) * pprob_weights)
end
...
Plots.plot!(time_vec, pβ; yerror =  weight_ci_len, label = "Group Aggregate", markerstrokecolor=:auto, linestyle = :dash, marker=(:circle,5), alpha = 1, dpi=600) 

plotpath = string(folder, "\\sel_grp_1.png")
group_plot
savefig(group_plot,  plotpath)
...
display(gcf())
plt.savefig(string(folder,"\\est_partition.png"))
