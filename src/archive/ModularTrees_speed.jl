...
function fit_grad!(ne::ols_grad, X::AbstractArray, Y::AbstractArray)
...
function predict_grad(ne::ols_grad, X::AbstractArray)
...
function fm_calc_mof!(fm::sse, Y::AbstractArray, Y_hat::AbstractArray)
...
function find_best_split(X::AbstractArray, Y::AbstractArray, Q::AbstractArray, feature_idx, minobs, ne_type, fm_type)
...
function compute_node!(tree::Tree, node::Node, X::AbstractArray, Y::AbstractArray, Q::AbstractArray, feature_idx, minobs, ne_type, fm_type)
...
function create_root_node!(X::AbstractArray, Y::AbstractArray, Q::AbstractArray, feature_idx, minobs, ne_type, fm_type)
...
function grow_tree(X::AbstractArray, Y::AbstractArray, Q::AbstractArray, feature_idx, minobs, ne_type, fm_type)
    comp = [""]
    s_time = time()
    if ndims(Q) == 1
...
function predict_single_tree(tree::Tree, X::AbstractArray, Q::AbstractArray)
...
function nfold_cross_val_prune(X::AbstractArray, Y::AbstractArray, Q::AbstractArray, regularize_final, nfolds, minobs, ne_type, fm_type)
    if ndims(Q) == 1
        Q = reshape(Q, 1, length(Q))
    end
