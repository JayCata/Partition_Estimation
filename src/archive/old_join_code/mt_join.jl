...
### Joining Functions ### 
#------------------------------------------------------#
function region_join(tree::Tree, X::AbstractArray, Y::AbstractArray)
    # Start timer
    s_time = time()
...
function rj_full_tree(tree::Tree, X::AbstractArray, Y::AbstractArray)
    n = length(Y)
    tree_list, group_list, ne_list, normalized_mof, indices = region_join(tree::Tree, X::AbstractArray, Y::AbstractArray)
    ## Standard
...
function val_rj(tree, regularize_final, X::AbstractArray, Y::AbstractArray)
    n = length(Y)
    tree_list, group_list, ne_list, normalized_mof, indices = region_join(tree::Tree, X::AbstractArray, Y::AbstractArray)
...
function nfold_cross_val_rj(X::AbstractArray, Y::AbstractArray, Q::AbstractArray, regularize_final, nfolds, minobs, ne_type, fm_type)
...
function init_classic_mat!(X, Y, criteria_mat, pot_idx, mof_list, num_groups, groups, ne_type, fm_type)
    # Running Objects
    fm_obj = fm_type()
    ne_obj = ne_type()
    for i in 2:num_groups
        for j in 1:i
            if pot_idx[i, j] != []
                ...
                Y_ij_hat  = predict(ne_obj, X[:, pot_idx[i, j]])
                fm_calc_mof!(fm_obj, Y[pot_idx[i, j]], Y_ij_hat)
                criteria_mat[i,j] = fm_obj.mof - unique(mof_list[groups .== i])[1] - unique(mof_list[groups .== j])[1]
            end
        end
    end
end

function update_classic_mat!(X, Y, criteria_mat, pot_idx, mof_list, num_groups, groups, ne_type, fm_type, update_index)
    fm_obj = fm_type()
    ne_obj = ne_type()
    i = update_index
    for j in 1:num_groups
        if pot_idx[i, j] != []
            ...
            criteria_mat[i, j] = fm_obj.mof - unique(mof_list[groups .== i])[1] - unique(mof_list[groups .== j])[1]
        elseif pot_idx[j, i] != []
            fit!(ne_obj, X[:, pot_idx[j, i]], Y[pot_idx[j, i]])
            Y_ij_hat  = predict(ne_obj, X[:, pot_idx[j, i]])
            fm_calc_mof!(fm_obj, Y[pot_idx[j, i]], Y_ij_hat)
            criteria_mat[j, i] = fm_obj.mof - unique(mof_list[groups .== j])[1] - unique(mof_list[groups .== i])[1]
        end
    end
end
...
function find_best_join!(X::AbstractArray, Y::AbstractArray, criteria_mat, pot_idx, mof_list, groups, ne_type, fm_type)
    ...
    fm_calc_mof!(fm_obj_opt, Y[pot_idx[mg_2, mg_1]], Y_ij_hat)
    fm_obj_opt.mof_gain = fm_obj_opt.mof - unique(mof_list[groups .== mg_2])[1] - unique(mof_list[groups .== mg_1])[1]
    return mg_1, mg_2, fm_obj_opt, ne_obj_opt
end

function region_join_redux(tree::Tree, X::AbstractArray, Y::AbstractArray, join_criteria = "classic")
    # Start timer
    s_time = time()
    ...
    while groups != Int.(ones(T))
        ...
        if init == true
            if join_criteria == "classic"
                init_classic_mat!(X, Y, criteria_mat, pot_idx, mof_leaves, num_groups, groups, tree.ne_type, tree.fm_type)
            elseif  join_criteria == "param_dist"
                #init_param_mat!(criteria_mat)
            end
            init = false
        else    
            if join_criteria == "classic"
               update_classic_mat!(X, Y, criteria_mat, pot_idx, mof_leaves, num_groups, groups, tree.ne_type, tree.fm_type, mg_1)
            elseif  join_criteria == "param_dist"
                #update_param_mat!(criteria_mat)
            end
        end
        # Find best join and output the two regions that are being joined
...
function rj_full_tree_redux(tree::Tree, X::AbstractArray, Y::AbstractArray)
    n = length(Y)
    tree_list, group_list, ne_list, normalized_mof, indices = region_join_redux(tree::Tree, X::AbstractArray, Y::AbstractArray)
    ## Standard
    #regularize_final = [x for x in range(0, stop = 1.1*(maximum(mof_gain_list)), length = 2 * length(tree_list))]
    #regularize_final = sort(unique(vcat(regularize_final, mof_gain_list)))
    #penalized_term = [x * y.regions for x = regularize_final, y in tree_list]
...
function val_rj_redux(tree, regularize_final, X::AbstractArray, Y::AbstractArray)
    n = length(Y)
    tree_list, group_list, ne_list, normalized_mof, indices = region_join_redux(tree::Tree, X::AbstractArray, Y::AbstractArray)
...
function nfold_cross_val_rj_redux(X::AbstractArray, Y::AbstractArray, Q::AbstractArray, regularize_final, nfolds, minobs, ne_type, fm_type)
    ...
    for i in 1:nfolds
        ...
        tree = grow_tree(X[:,train_ind], Y[train_ind], Q[:, train_ind], feature_idx, minobs, ne_type, fm_type)
        # Prune tree based on full tree alpha levels
        val_trees = val_rj_redux(tree, regularize_final, X[:, train_ind], Y[train_ind])
        nval = length(Y[train_ind])
        predict_val(x) = predict_single_tree(x, X[:,fold_vec[i]], Q[:,fold_vec[i]])
        Y_hat_list = predict_val.(val_trees)
        for j in 1:length(regularize_final)
